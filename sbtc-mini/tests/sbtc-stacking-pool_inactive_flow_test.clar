...
(define-public (test-inactive-state)
	(begin
		...
		(try! (check-sign-pre-register-inactive))
        ;; @mine-blocks-before 2100
		(try! (check-sign-register-inactive))
        ;; @mine-blocks-before 1600
		(try! (check-vote-inactive))
        (ok true))
)

(define-public (check-sign-pre-register-inactive)
    (let
        ((registration-result
...
(define-public (check-sign-register-inactive)
    (let
        ((registration-result
...
(define-public (check-vote-inactive)
    (let
        ((vote-result
			...
			(asserts! (is-eq vote-result err-not-enough-locked-stx) (err (unwrap-err-panic vote-result)))
			(ok true)))
