...
;; @name user can't pre-register with to little stx (10 STX)
;; @caller wallet_1
(define-public (test-sign-pre-register-with-low-amount)
	(begin
		;; @continue
		...
		(try! (check-sign-pre-register-too-little-stx))
		(ok true)
	)
)


(define-public (check-sign-pre-register-too-little-stx)
	(let
		((registration-result
				(contract-call? .sbtc-stacking-pool signer-pre-register u10000000 mock-pox-reward-wallet-1)))
			(asserts! (is-err registration-result) err-error-expected)
			(asserts! (is-eq registration-result err-not-enough-stacked) registration-result)
			(ok true)))
